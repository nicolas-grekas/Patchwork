...
class patchwork_PHP_iconv
{
    const

    ERROR_ILLEGAL_CHARACTER = 'patchwork_PHP_iconv::iconv(): Detected an illegal character in input string',
    ERROR_WRONG_CHARSET     = 'patchwork_PHP_iconv::iconv(): Wrong charset, conversion from `%s\' to `%s\' is not allowed';
...
/**/{
        ...
        function iconv($in_charset, $out_charset, $str) {return patchwork_PHP_iconv::iconv($in_charset, $out_charset, $str);}
        function iconv_mime_decode_headers($encoded_headers, $mode = 2, $charset = INF) {return patchwork_PHP_iconv::mime_decode_headers($encoded_headers, $mode, $charset);}
        function iconv_mime_decode        ($encoded_headers, $mode = 2, $charset = INF) {return patchwork_PHP_iconv::mime_decode        ($encoded_headers, $mode, $charset);}
        function iconv_get_encoding($type = 'all')   {return patchwork_PHP_iconv::get_encoding($type);}
        function iconv_set_encoding($type, $charset) {return patchwork_PHP_iconv::set_encoding($type, $charset);}
        function iconv_mime_encode($field_name, $field_value, $pref = INF) {return patchwork_PHP_iconv::mime_encode($field_name, $field_value, $pref);}
        function ob_iconv_handler($buffer, $mode)  {return patchwork_PHP_iconv::ob_handler($buffer, $mode);}
        function iconv_strpos ($haystack, $needle, $offset = 0, $encoding = INF) {return patchwork_PHP_iconv::strpos ($haystack, $needle, $offset, $encoding);}
        function iconv_strrpos($haystack, $needle,              $encoding = INF) {return patchwork_PHP_iconv::strrpos($haystack, $needle,          $encoding);}
        function iconv_substr($s, $start, $length = PHP_INT_MAX, $encoding = INF) {return patchwork_PHP_iconv::substr($s, $start, $length, $encoding);}

        if (extension_loaded('xml'))
        {
            function iconv_strlen($s, $encoding = INF) {return patchwork_PHP_iconv::strlen1($s, $encoding);}
        }
        else
        {
            function iconv_strlen($s, $encoding = INF) {return patchwork_PHP_iconv::strlen2($s, $encoding);}
        }
/**/}
